...
def build_graph(path):
    ...
    # pos = nx.spring_layout(G.to_networkx(), seed=42)
    # nx.draw(G.to_networkx(), pos, with_labels=True, node_size=1500, node_color='skyblue', font_size=10,
    #         font_weight='bold', alpha=0.8, edge_color='gray')
    # plt.show()
...
def GraphTrain(data, labels, graph):
    ...
    # category_output, _ = combined_model(graph, graph.ndata['x'])
    # predicted_categories = torch.argmax(category_output[train_mask].squeeze(), dim=1)

    torch.save(combined_model.state_dict(), 'bcc_model.ckpt')
...
def GraphTest(data, labels, graph):
    ...
    # test_mask = range(168, 209)  # 从169开始

    with torch.no_grad():
        combined_model.eval()
        combined_model.load_state_dict(torch.load('bcc_model.ckpt'))
        ...
        acc = accuracy_score(predicted, labels[test_mask].view(-1))
        prec = precision_score(predicted, labels[test_mask].view(-1))
        reca = recall_score(predicted, labels[test_mask].view(-1))
        f1 = f1_score(predicted, labels[test_mask].view(-1))
        cin = c_index(predicted, labels[test_mask].view(-1))

        score = Score(data[test_mask].numpy().squeeze(), max_values, min_values, w)
        print(score)
        print(labels[test_mask].view(-1))

        scin = c_index(score, labels[test_mask].view(-1))
        print(scin)

        return max_values, min_values, acc, prec, reca, f1, cin, scin
...
if __name__ == '__main__':
    ...
    acc_list = [0 for x in range(0, num_runs)]
    prec_list = [0 for x in range(0, num_runs)]
    reca_list = [0 for x in range(0, num_runs)]
    f1_list = [0 for x in range(0, num_runs)]
    ...
    for i in range(num_runs):
        ...
        data_path = 'dataset/BCC.txt'
        x, y, g = build_graph(data_path)
        GraphTrain(x, y, g)

        w = WeightTrain(x, y)
        max_list[i], min_list[i], acc_list[i], prec_list[i], reca_list[i], f1_list[i], cin_list[i], scin_list[i], \
                    = GraphTest(x, y, g)

        # print(acc_list[i], prec_list[i], reca_list[i], f1_list[i], cin_list[i], scin_list[i])

    av_acc = np.mean(acc_list)
    av_prec = np.mean(prec_list)
    av_reca = np.mean(reca_list)
    ...
    std_reca = np.std(reca_list)
    std_f1 = np.std(f1_list)
    std_cin = np.std(cin_list)
    std_scin = np.std(scin_list)

    max_scin = np.max(scin_list)

    # print("Res:", " Accuracy {:.4f} | Precision {:.4f} | Recall {:.4f} | F1 Score {:.4f} | C-index {:.4f} \
    #               | SC-index {:.4f} | MSC-index {:.4f}".format(av_acc, av_prec, av_reca, av_f1, av_cin, av_scin,
    #                                                            max_scin))
    #
    # print("Dev:", " Accuracy {:.4f} | Precision {:.4f} | Recall {:.4f} | F1 Score {:.4f} | C-index {:.4f} \
    #               | SC-index {:.4f}".format(std_acc, std_prec, std_reca, std_f1, std_cin, std_scin))
    end_time = time.time()
